...
При старте Railway сначала выполняется:

```bash
alembic upgrade head
```

После миграций запускается FastAPI/бот. Для уже созданной Railway-базы initial migration написана безопасно: она не пересоздает существующие таблицы.
